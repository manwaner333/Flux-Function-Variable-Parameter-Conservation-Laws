...
def f_real_function(u):
    f = 0.5 * u * (3 - np.power(u, 2)) + (1/12) * 130 * np.power(u, 2) * ((3/4) - 2*u +(3/2)*np.power(u, 2) - (1/4)*np.power(u, 4))
    return f
def f_predict_function(u):
    # molecular = (1.5565419434587793)*u+(-1.0971840954891203)*1+(-0.8876336823197226)*u**2+(-0.032719609718727206)*u**3+(-0.01458867599399175)*u**4+(-0.002044326714069395)*u**5+(-5.2542200482669494e-05)*u**6
    # denominator = (0.705292220207145)*1+(0.26395864168182653)*u+(-0.1591974300583845)*u**2+(-0.006187855496516052)*u**4+(-0.0008573298819781676)*u**5+(-0.0003693852255369223)*u**3+(-2.2034637726282318e-05)*u**6
    ...
    molecular = (1.713990898734339)*u+(-1.4919981515720633)*1+(-1.1705309989662056)*u**2+(0.05370336004192864)*u**3+(-0.02894116174581749)*u**4+(0.00047784753575213465)*u**5+(-1.908531335257056e-06)*u**6
    denominator = (0.7222416505158625)*1+(0.23060678975614496)*u+(-0.14943207731053496)*u**2+(0.01788730689053679)*u**3+(-0.010962610467067996)*u**4+(0.0001809886655490585)*u**5
...
def plot_real_and_predict_function(name='name'):
    ...
    # if not os.path.isdir(save_dir):
    #     os.mkdir(save_dir)
    # file_name = '/' + 'real_prediction_function_1' + '.pdf'
    # fig.savefig(save_dir + file_name, bbox_inches='tight', pad_inches=0.5)
...
if __name__ == "__main__":
    ...
    test_data_name = 'N_400_example_6_dt_0.1_layer_10_beta_130'
