...
def f_predict_function(u):
    # f = u ** 4 / u ** 2 - u ** 4 * u / u ** 4 * u * u ** 2 + u ** 2 / u * u
    # f = u*u-u**4/u**2*-0.1039
    # eq = 'x**6-x**5*x**4-x**6*x**4*0.5112130536642007*0.6930707034561605*0.9771519979673348+0.11678507819925954-0.8246847486455432+0.1810478367849192-x**4+0.05505348484433392+x**4*x**5/x**6+x**6*x**6*0.4898228809753291-0.974364302050385*0.6467801862153'
    # eq_str = eq.replace('x', 'u')
    # f_str = sympy.sympify(eq_str)
    # f = sympy.lambdify(sympy.Symbol('u'), f_str)
    c = np.array([[-0.2890, -0.0702, -0.1474,  0.3735, -0.1713,  1.1265,  0.0422,  0.0736]])
    f = u**7*c[0,0] + u**6*c[0,1] + u**5*c[0,2] + u**4*c[0,3] + u**3*c[0,4] + u**2*c[0,5] + u*c[0,6] + c[0,7]
    return f
...
if __name__ == "__main__":
    ...
    # predict_data_file = 'data/' + 'predicted_' + test_data_name + '_U' + '.npy'
    #
    # real_time_step = 167
    # predict_time_step = 167
    # time = 20
    # example_index = 0
