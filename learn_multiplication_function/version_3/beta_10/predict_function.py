...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        ...
        m = (1.7006642479391998)*u+(-1.6452598353864365)*1+(-0.9640845280843208)*u**3+(0.32141408878787053)*u**4+(-0.019774328349539017)*u**2+(-0.0035837572880650964)*u**6+(-0.0029507141087210752)*u**5+(0.0029023054632591077)*u**7+(-0.00048249102443053484)*u**8
        d = (1.0205228295019033)*1+(-0.06627542485672068)*u+(0.043123192645832815)*u**2+(0.002539329424290226)*u**3+(0.0017753486411678139)*u**6+(0.0014617469494484089)*u**5+(-0.0014377659105460562)*u**7+(-0.001274020362540719)*u**4+(0.00023902003281614455)*u**8
        return m/d
...
if __name__ == "__main__":
    experiment_name = 'N_400_example_1_dt_0.1_layer_10_beta_10'
    real_data_file = 'data/predicted_' + experiment_name + '_U' + '.npy'
