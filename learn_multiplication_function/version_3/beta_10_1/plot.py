...
def f_predict_function(u):
    # molecular = (42.61726878223532)*u**6+(28.660975444428143)*u**5+(18.854262366374243)*u**7+(16.094721778893952)*u**2+(-12.40831386864542)*u**4+(-7.03527368936129)*u+(2.813685852447237)*u**8+(1.0761618530944248)*1+(-0.01791709031815536)*u**3
    # denominator = (31.754077807892187)*u**6+(21.355259741371036)*u**5+(14.048289139585233)*u**7+(11.279211504854608)*u**2+(-8.799688956617102)*u**4+(-8.446993796141012)*u+(3.1695595956578138)*1+(2.096474082891445)*u**8+(1.4800535021849823)*u**3
    molecular = (1.7418195544381319)*u+(-1.483932990628753)*1+(-1.1083654288440252)*u**3+(0.43122977236754795)*u**4+(-0.02751995494422027)*u**2+(-0.003993980054320638)*u**6+(0.0039774839191152635)*u**7+(-0.0036448365377201694)*u**5+(-0.0007723325171523393)*u**8
    denominator = (1.0131816718187407)*1+(-0.12366504115690086)*u+(0.06951187692511271)*u**2+(0.020565843829340007)*u**3+(-0.009303329528139451)*u**4+(0.0066265585602135755)*u**6+(-0.006599189218236678)*u**7+(0.006047281766838241)*u**5+(0.0012814051605817725)*u**8
...
if __name__ == "__main__":
    N = 400
    test_data_name = 'N_400_example_1_dt_0.1_layer_10_beta_10'
    ...
    # 画出观测数据得分布图
    # time_point_list = [0, 21, 42, 63, 84, 105, 126, 147, 168, 189]
