...
class poly(torch.nn.Module):
    def __init__(self, hidden_layers, channel_num, channel_names=None, theta=0.1, normalization_weight=None):
        ...
        # molecular
        module = torch.nn.Linear(8, 1).to(dtype=torch.float64)
        module.weight.data.fill_(0)
        module.bias.data.fill_(0)
        self.add_module('molecular', module)
        layer.append(self.__getattr__('molecular'))
        # denominator
        # module = torch.nn.Linear(5, 1).to(dtype=torch.float64)
        # module.weight.data.fill_(0)
        # module.bias.data.fill_(0)
    ...
    def forward(self, inputs):
        ...
        outputs_m = torch.cat([v_1_m, v_2_m, v_3_m, v_4_m, v_5_m, v_6_m, v_7_m, v_8_m], dim=-1)

        # 除法
        r1 = self.layer[-1](outputs_m)
        return r1[...,0]
