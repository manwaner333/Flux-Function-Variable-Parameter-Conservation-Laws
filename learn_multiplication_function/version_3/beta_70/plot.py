...
def f_real_function(u):
    f = 0.5 * u * (3 - np.power(u, 2)) + (1/12) * 70 * np.power(u, 2) * ((3/4) - 2*u +(3/2)*np.power(u, 2) - (1/4)*np.power(u, 4))
    return f
def f_predict_function(u):
    molecular = (1.2015465258611915)*u+(-0.9205793413186155)*1+(-0.4673232317185197)*u**2+(-0.16530615673809193)*u**3+(0.006528499600684442)*u**4+(-0.0027200761144343653)*u**5+(-0.0003224814707197677)*u**6+(4.060877862560292e-05)*u**7
    denominator = (0.6675410783038058)*1+(0.3295479387907382)*u+(-0.07944104572486198)*u**2+(-0.03264883628555068)*u**3+(0.0020190324373936513)*u**4+(0.0013302190193921768)*u**5+(0.00015770550812038586)*u**6+(-1.9859212540196347e-05)*u**7
...
if __name__ == "__main__":
    ...
    test_data_name = 'N_400_example_4_dt_0.1_layer_10_beta_70'
