...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        f = 0.5 * u * (3 - torch.pow(u, 2)) + (1/12) * 70 * torch.pow(u, 2) * ((3/4) - 2*u + (3/2)*torch.pow(u, 2) - (1/4)*torch.pow(u, 4))
        return f
    ...
    def forward(self, init, stepnum):
        ...
        for i in range(1, stepnum):
            ...
            # print(torch.min(u_old, 1).values, torch.max(u_old, 1).values)
...
if __name__ == "__main__":
    experiment_name = 'N_400_example_6_dt_0.1_layer_10_beta_70'
