...
def f_real_function(u):
    f = 0.5 * u * (3 - np.power(u, 2)) + (1/12) * 50 * np.power(u, 2) * ((3/4) - 2*u +(3/2)*np.power(u, 2) - (1/4)*np.power(u, 4))
    return f
def f_predict_function(u):
    molecular = (1.4158193455928507)*u+(-1.3127289843002734)*1+(-0.41388731833943065)*u**2+(-0.30564908473918917)*u**3+(0.029674665581297985)*u**4+(-0.0050219536872089686)*u**5+(-0.0010666645852766112)*u**6+(0.0002706643545234682)*u**7+(-1.2339766104301593e-05)*u**8
    denominator = (0.8068155754095208)*1+(0.17807137418938865)*u+(-0.034527681259374166)*u**3+(-0.033262970839660636)*u**2+(0.0028964149843736155)*u**4+(0.000700679096203644)*u**5+(0.00014882446635612533)*u**6+(-3.776395942979052e-05)*u**7+(1.7216837708695947e-06)*u**8
...
if __name__ == "__main__":
    ...
    test_data_name = 'N_400_example_8_dt_0.1_layer_10_beta_50'
