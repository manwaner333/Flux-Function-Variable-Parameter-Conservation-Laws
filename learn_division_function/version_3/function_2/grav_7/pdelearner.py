...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        f = (torch.pow(u, 2) * (1 - 7 * torch.pow(1-u, 4))) / (torch.pow(u, 2) + 0.5 * torch.pow(1-u, 4))
        # f = 0.5 * u * (3 - torch.pow(u, 2)) + (1/12) * 130 * torch.pow(u, 2) * ((3/4) - 2*u + (3/2)*torch.pow(u, 2) - (1/4)*torch.pow(u, 4))
        return f
    ...
    def f_half(self, u):
        ...
        for index in range(self.N - 1):
            ...
            # f_half[:, index] = 0.5 * (f[:, index] + f[:, index + 1]) - 0.5 * torch.max(torch.max(dfdu), torch.from_numpy(np.array([0.001]))) * (u[:, index + 1] - u[:, index])
...
def generate_real_data(save_file, u0_file, u_fixed_file):
    ...
    # batch_size = 1
    # u_0_np = np.zeros((batch_size, N), dtype=float)
    # u_0_np[0:1, 180:240] = 1-0.1
    # u_0_np[0:1, 0:180] = 0.1
    # u_0_np[0:1, 240:400] = 0.1
    # u_0 = torch.from_numpy(u_0_np)
    # u_0 = u_0.to(device)
...
if __name__ == "__main__":
    experiment_name = 'N_400_example_5_dt_0.1_layer_10_grav_7_test'
