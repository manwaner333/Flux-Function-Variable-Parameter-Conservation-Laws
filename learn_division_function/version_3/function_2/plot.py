...
def plot_fixed_time_step(real_data, predict_data, time_steps_real, time_steps_predict, time, example_index, name):
    ...
    for i in range(len(fix_timestep_real_data)):
        x_label.append(i * dx)
    # 进行一个去掉负数的处理
    fix_timestep_real_data = [i if i >= 0.0 else 0.0 for i in fix_timestep_real_data]
    fix_timestep_predict_data = [i if i >= 0.0 else 0.0 for i in fix_timestep_predict_data]
...
if __name__ == "__main__":
    ...
    # # 画出不同时刻的预测和真实的解
    beta_list = [-3]   # [-3, -1, 9]
    for power_nb in [3, 4, 5]:  # [3, 4, 5, 6, 7]
