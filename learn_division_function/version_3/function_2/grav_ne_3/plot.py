...
def f_real_function(u):
    f = (np.power(u, 2) * (1 - (-3.0) * np.power(1-u, 4))) / (np.power(u, 2) + 0.5 * np.power(1-u, 4))
    return f
def f_predict_function(u):
    # molecular = (-1.249624970255768)*1+(1.2469669777395187)*u+(-0.8868916488353695)*u**2+(-0.12286585055738726)*u**3+(0.00616208595876208)*u**4+(-0.00033716725126856914)*u**5+(-3.20045324559558e-06)*u**6+(2.5929408460456508e-06)*u**7
    # denominator = (0.6612305717871988)*u+(0.6337514776621572)*1+(-0.24084717762026112)*u**2+(-0.02969018266114539)*u**3+(-0.003770826294721476)*u**4+(-0.0018791972111883797)*u**5+(-1.783768379352881e-05)*u**6+(1.4451721477493489e-05)*u**7
    molecular = (3.731053839218556)*u+(1.2072026802513816)*1+(-1.1190534843215094)*u**2+(-0.05985997573231876)*u**3+(0.012415418746051149)*u**4+(-0.00021387599125998166)*u**5+(-2.2249125443332137e-05)*u**6
    denominator = (1.1930211870346836)*1+(0.8541807879095531)*u**2+(-0.1031241847375493)*u**3+(-0.05415640968478909)*u+(0.010489061335074023)*u**4+(-0.00025341637329727234)*u**5+(-2.2327743188645476e-05)*u**6
...
if __name__ == "__main__":
    ...
    test_data_name = 'N_400_example_9_dt_0.1_layer_10_grav_ne_3'
    ...
    # 画出观测数据得分布图
    # time_point_list = [0, 16, 32, 49, 65, 81, 97, 113, 130, 146]
