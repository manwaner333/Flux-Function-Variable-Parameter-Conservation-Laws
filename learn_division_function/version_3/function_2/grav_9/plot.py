...
def f_real_function(u):
    f = (np.power(u, 2) * (1 - 7.0 * np.power(1-u, 4))) / (np.power(u, 2) + 0.5 * np.power(1-u, 4))
    return f
def f_predict_function(u):
    molecular = (2.6164880268604622)*u**2+(1.0810906224489698)*u**4+(-0.8020255155166863)*u+(-0.47894736023327833)*u**3+(0.395178612705052)*u**5+(-0.1287670510022127)*1+(0.04963702519451369)*u**6+(0.0027281543459820694)*u**7+(5.624169285111473e-05)*u**8
    denominator = (4.220220532936433)*u**2+(-3.2648415566100524)*u+(0.9915747630095805)*u**4+(0.9134969308820331)*1+(0.35731179263503754)*u**5+(-0.09164570816044737)*u**3+(0.04488070427677644)*u**6+(0.002466737036387849)*u**7+(5.0852499217764655e-05)*u**8
...
if __name__ == "__main__":
    ...
    test_data_name = 'N_400_example_9_dt_0.1_layer_10_grav_9'
    ...
    # 画出观测数据得分布图
    time_point_list = [0, 28, 56, 85, 113, 141, 169, 198, 226, 254]
