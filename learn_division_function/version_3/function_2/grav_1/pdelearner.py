...
class VariantCoeLinear1d(torch.nn.Module):
    ...
    def f_real(self, u):
        f = (torch.pow(u, 2) * (1 - 1.0 * torch.pow(1-u, 4))) / (torch.pow(u, 2) + 0.5 * torch.pow(1-u, 4))
        return f
...
def generate_real_data(save_file, u0_file, u_fixed_file):
    ...
    u_0_np[3:4, 0:100] = 0.2
    u_0_np[3:4, 180:400] = 0.2
    u_0_np[4:5, 100:180] = 0.6
    u_0_np[4:5, 0:100] = 0.2
    u_0_np[4:5, 180:400] = 0.2
...
if __name__ == "__main__":
    experiment_name = 'N_400_example_6_dt_0.1_layer_10_grav_1_test'
