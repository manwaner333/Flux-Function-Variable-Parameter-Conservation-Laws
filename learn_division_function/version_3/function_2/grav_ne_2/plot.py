...
def f_real_function(u):
    f = (np.power(u, 2) * (1 - (-2.0) * np.power(1-u, 4))) / (np.power(u, 2) + 0.5 * np.power(1-u, 4))
    return f
def f_predict_function(u):
    # molecular = (-24.127813884064942)*u**3+(12.931564291345246)*u**2+(9.848022725694193)*u**4+(-5.712596612621818)*1+(3.1662001257583694)*u+(-2.775527390019831)*u**6+(2.5163640351994583)*u**5+(1.82747382110081)*u**7+(-0.4362635791427192)*u**8
    # denominator = (-3.2827401240788503)*u**6+(2.9762160571145384)*u**5+(-2.819456742942237)*u**4+(2.161434853715661)*u**7+(1.7640648449537786)*1+(1.7638401767998413)*u**3+(-1.2931419455831517)*u+(-0.5159884067711618)*u**8+(0.5146623352524677)*u**2
    molecular = (2.845366347505652)*u+(1.4682046039782792)*1+(-0.41277794584600747)*u**2+(-0.07736973801525364)*u**3+(-0.017995776240922946)*u**4+(-0.001360591517600178)*u**5+(2.4138643235325388e-05)*u**7+(1.5440270086212782e-05)*u**6+(2.163326541059468e-06)*u**8
    denominator = (1.0396831771374555)*1+(0.5536483273501103)*u**2+(-0.045496720084852685)*u+(0.029935831467234084)*u**3+(-0.00839028141628346)*u**4+(-0.0026688777435316183)*u**5+(-6.597290224302406e-05)*u**6+(2.1822294341745827e-05)*u**7+(2.7851608318026166e-06)*u**8
...
if __name__ == "__main__":
    ...
    test_data_name = 'N_400_example_3_dt_0.1_layer_10_grav_ne_2'
    ...
    # 画出观测数据得分布图
    # time_point_list = [0, 19, 37, 56, 74, 93, 112, 130, 149, 167]
    # time_point_list = [0, 9, 19, 28, 37, 47, 56, 65, 74, 84, 93, 102, 112, 121, 130, 140, 149, 158, 167, 177]
